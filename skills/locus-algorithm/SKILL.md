...
At every phase transition, produce visible structured output — the phase's findings, the decisions it reached, and the next step it points to — so the user can follow the work and see progress.
...
Every Algorithm-required output — the reverse-engineering, the ISC criteria, the risks, the plan, the phase progress and the verification evidence — MUST appear in the visible response, so the user can trace how the work moved through the problem. Silent execution of Algorithm phases is a failure: the user cannot follow the work, or verify compliance, with what they cannot see.
