...
| Tier            | Time Budget | ISC Range | Min Capabilities | When                                          |
|-----------------|-------------|-----------|------------------|-----------------------------------------------|
| **Minimal**     | <1 min      | 3-6       | 0-1              | Single, well-understood task                  |
| **Standard**    | <5 min      | 4-10      | 1-2              | Normal request (DEFAULT)                      |
| **Extended**    | <15 min     | 6-16      | 3-5              | Quality must be extraordinary, multi-system   |
| **Advanced**    | <30 min     | 8-24      | 4-7              | Substantial multi-file work                   |
| **Deep**        | <60 min     | 10-32     | 6-10             | Complex design, cross-cutting refactors       |
| **Comprehensive** | <120 min  | 12-40     | 8-15             | Major architectural work, no time pressure    |

**ISC Range** = a floor *and* a ceiling. The floor forces search past the first plausible
decomposition; the ceiling stops the count becoming the goal. Land between them, or record
an exception (see the ISC Count Gate). The right number of criteria is however many
independently-failing states the Splitting Test actually finds — the range is a sanity
check on that number, never a substitute for it.

> **These ranges are provisional.** They are calibrated by argument, not measurement.
> **DEV-515** (falsification spike) should set them empirically; until it reports, treat
> these as a conservative cap rather than a validated constant, and prefer recording an
> exception over padding to fit.
...
**Below the floor** — decompose further before exiting OBSERVE. The floor exists because
models otherwise stop at the first plausible decomposition; being under it is evidence the
search stopped early, not a formatting problem.

**Above the ceiling** — one of three things is true, and you must say which:
1. The tier is wrong. Reclassify upward and re-check against the new range.
2. Criteria are being padded — split for the count rather than because the parts fail
   independently. Merge them back.
3. The work genuinely has that many independently-failing states. Record an exception.

**Recording an exception.** Write one line under `## Decisions` in the PRD:

```
ISC EXCEPTION: {count} criteria vs {tier} range {floor}-{ceiling} — {reason}
```

A recorded exception is a valid exit from OBSERVE. An unrecorded count outside the range is
not. The point is not to make the range unbreakable; it is to make breaking it *visible* and
*argued*, so DEV-515 has real cases to calibrate against instead of a compliance number.

**What the range is not.** It is not the quality bar. The Splitting Test above does the real
work: it decides whether a criterion is atomic, and atomicity is what makes verification
real. Criteria written to reach a count are worse than no criteria, because they carry the
appearance of rigour without the substance — the same failure the Phantom Capability Rule
names.
...
`## Decisions` is also where an **ISC exception** is recorded when the criteria count falls
outside the tier's range — one line, with its reason. See the ISC Count Gate above.
...
- If context gathering requires reading 3+ files and the work warrants delegation, dispatch a session: `allele_sessions_create` when Allele is reachable, otherwise route down the table in [Dispatch](#dispatch). You do not have to guess which vehicles are up — the `SessionStart` notice states it once per session.
...
**Effort level:** Select based on the request analysis. Default Standard. Announce the tier and a one-sentence rationale.
...
**One `allele_sessions_create` at a time. Read its result before issuing the next.** This
is a rule, not a default, and it has no exception clause. Batching creates into a single
assistant message is the thing that broke: on 2026-09-12 an overlapping create returned
*another caller's* `session_id`. Nothing in the response said so. Dispatch attribution was
filed against the wrong id — five sessions then refused `allele_sessions_discard` and had to
be removed by hand — submit retries were typed into a different session's terminal, and one
worker was handed the session id of another worker's reviewer.

**Serialising the creates does not serialise the work.** The workers still run concurrently;
only the *claims* queue. Twelve creates at roughly a second each add about eleven seconds to
a run whose workers take minutes. That asymmetry is the entire argument: being wrong about
serialising costs seconds, being wrong about batching costs mis-attributed sessions and
manual cleanup. Anyone who later reads this and spots an obvious parallelism win has found
the same win that caused the incident.

It is also what makes the two limits below countable. An orchestrator that fired creates
blind does not know how many slots it holds, and cannot reclaim what it cannot name — so
this rule stands on its own even if the race never recurs.

**Why there is no exception clause.** An exception would ask the orchestrator to judge, at
dispatch time, whether batching is safe here. The fact that would settle it — whether the
allele on the other end claims ids race-free — is not observable from inside the session,
and a skill ships to whatever allele is installed. allele's id inference was fixed in
DEV-601; this rule deliberately does not depend on that fix, because nothing in a skill can
check it. **Retirement condition:** remove this rule when `allele_sessions_create` is
specified and tested to be safe under concurrent callers, and Locus can tell which allele it
is talking to. Remove it deliberately, with that evidence — not because it looks slow.
...
**Every session you dispatch is either discarded or explicitly recorded as still working
with a reason.** A skill that fans out says how it reclaims. Reclamation is part of the
dispatch, not tidying afterwards.

**When not to dispatch.** Trivial lookups, anything already in this context, and work whose
intermediate steps you need to watch directly. Dispatch is for work with its own
workspace, its own branch, or its own perspective.

**Which vehicle.** Allele is the default and the preference is strong, for the reason at
the top of this section. But a preference and a prohibition are different things, and only
the first has defined behaviour when it cannot be honoured. Route down this table and stop
at the first row that is available:

| Tier | Vehicle | When | What it costs you |
|---|---|---|---|
| **1 — happy path** | `allele_sessions_create` | The Allele app is reachable. Always prefer it. | Nothing. |
| **2 — fallback** | `locus delegate run --backend opencode` | Allele unavailable. | No workspace, no branch, no conversation, read-only. Say so. |
| **3 — last resort** | native `Task` / `Agent` | Both of the above unavailable. **Explicitly permitted.** | Hidden, unaddressable, uninterruptible. State the degradation before you dispatch. |
| **4** | this session, inline | The work never warranted delegation. | Orchestrator context. |
| **terminal** | stop and say delegation is unavailable | The work needs a real session and none exists. | Nothing. This is a correct answer, not a failure. |

**Tier 4 is not the bottom of the table, and the distinction is load-bearing.** "It did not
need delegating after all" is a judgement about the work. Made *after* the first three tiers
fail, it is marking your own homework — the conclusion is fixed before the reasoning starts,
and nobody outside the session can tell the difference. Decide whether the work warrants
delegation before you try to dispatch it. If it did, and no vehicle is available, the
terminal row is the honest one: say delegation is unavailable, name what you would have
dispatched, and let the user decide. Quietly absorbing a large parallel workload into this
session because nothing else was reachable is the original failure wearing a permitted
label.

**Busy is not absent.** A running app with every dispatch slot taken is *reachable*, and
reachable means tier 1. Wait, or reclaim a slot with `allele_sessions_discard`. Slot
pressure is backpressure and must never unlock a less observable vehicle — if it did, the
busier the system got the more of its work would route to the vehicle nobody can see, and
the sessions that remained visible would be a biased sample of the cheap ones.

**Tier 2 cannot do write work.** `locus delegate run` is read-only, so for anything that
edits files the table is two rows deep, not four: a real session, or a hidden one. Weigh
tier 3 accordingly — a subagent editing a branchless tree is precisely the case the
argument at the top of this section was written about.

**The hook enforces this, and it fails open.** `PreToolUse` denies a native `Task` while a
higher-priority vehicle is actually reachable, and names the one it is routing you to. When
neither sanctioned vehicle is reachable it does not deny at all: denying with nowhere to
send you is a dead end, and that dead end is the defect this table replaced. The hook probes
the machine; you observe your own toolset. When the two disagree — the Allele app is up, but
this session has no `allele_*` tools — you hold the better evidence. Say so, and re-issue
the call with the **description starting with** `locus:no-allele`, followed by your reason.
It has to be the first thing in the field: the marker appears in this document, so a
mention anywhere else is read as a quotation rather than an assertion. That releases the
denial, and records both the assertion and the reason you gave.

**This is not a reversal, and the record should say so plainly**, because in six months
somebody will read the table and assume the ban was lost in a moment of weakness. It was
not. The argument for Allele is unchanged and is still the whole reason for the preference:
a real session is interruptible and addressable, and a subagent is neither. What changed is
only what happens when the preference cannot be honoured. The prohibition this replaced had
no terminal state, and on 2026-08-30 that produced a live dead end — the allele MCP
unregistered, `locus delegate run` failing 401, native subagents forbidden, and nothing
anywhere saying that delegation had become impossible. Prefer Allele, hard. Permit a
subagent when Allele and OpenCode are both gone. Announce the degradation either way.
...
3. **ISC Count Gate is mandatory** — land inside the tier's ISC range, or record an exception in the PRD's `## Decisions` with its reason. Never pad to a number.
...
At every phase transition, write a short record of the work into the response: which phase you are in, what it found, what was decided, the evidence behind it, and the next step.

Each phase record shows:
- Phase name and number (e.g., "Phase 1: OBSERVE (1/7)")
- The deliverables defined for that phase (request analysis, ISC criteria, risks, plan, etc.)
- A brief progress summary before moving to the next phase

The Algorithm's deliverables — the request analysis, the ISC criteria, the risks, the plan, the phase progress and the verification evidence — MUST appear in the response as a record of the work, so the user can check what was done and what it rests on. A phase that runs with no record in the response is a failure: the user cannot check progress, or verify compliance, from a record that isn't there.
