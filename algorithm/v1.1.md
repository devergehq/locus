...
Every capability selected in OBSERVE creates a binding commitment to **invoke it via a tool call** — `Skill`/Read tooling for skills, `locus delegate run` via Bash for agent delegation. There is no text-only alternative.
...
- If context gathering requires reading 3+ files, delegate via `locus delegate run` if delegation is needed. Do not use platform-native Task/Agent subagents; if Locus Delegate is unavailable, continue serially or ask the user.
...
**Invoke each selected capability via tool call** — `Skill`/Read tooling for skills, `locus delegate run` via Bash for agent delegation. Every capability listed in OBSERVE must have a corresponding tool call in BUILD or EXECUTE. No exceptions.

- Any preparation work (scaffolding, research, design) happens here.
- Delegate investigative subtasks via `locus delegate run`; keep build artefacts in main context, exploration out.
...
**Capability invocation check:** for every capability selected in OBSERVE, confirm it was actually invoked via `Skill`/Read tooling or `locus delegate run`. Text output alone does not count. If any selected capability lacks a tool call, flag it as a failure.
