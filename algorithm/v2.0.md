...
- Recall relevant project memory from `{data}/projects/` — this is the canonical, structured store and it *is* readable. There is no retrieval path for `memory/learning/`; do not narrate recalling one (see `DECISIONS.md`, DEV-516).
- Read relevant source files (targeted, not exploratory sweeps).
- If context gathering requires reading 3+ files, move it out of this session — route it per the table in the Dispatch section below. If the allele MCP is unavailable, fall back to `locus delegate run` or a native subagent as the routing table directs; if neither fits, continue serially or ask the user.
...
**Answer these in the visible response.** Reflection is the deliverable — it informs the
session that produced it and the human reading it.

**Do not file them to `{data}/memory/learning/`.** That write was mandatory for 371
documents and nothing ever read one back; the persistence step was removed under DEV-516.
See `DECISIONS.md` for the decision and its reversal condition.

**What to persist instead**, when a learning outlives this session:

- **Durable project knowledge** → `{data}/projects/{slug}/` — the canonical store, small,
  structured, and actually read back.
- **A decision worth inheriting** → a record in the repository's `DECISIONS.md`.
- **A defect or a gap in Locus itself** → an issue, or `FUTURE_GAPS.md`.

Each of these has a consumer. A learning with no consumer is not persisted; it is discarded
in a directory.

**Research sessions are unchanged.** If this session used the Research skill (any mode),
still write the research artifact to
`{data}/memory/research/{YYYY-MM}/{YYYYMMDD}-{HHMMSS}_{slug}.md`. DEV-516 covers
`memory/learning/` only; `memory/research/` is a separate tree and is not affected.
...
**Delegate to find, dispatch to fix, and never let one session both diagnose and repair its
own diagnosis.**

### If the `allele_*` tools are absent

allele is not running and this session is outside it. The **Dispatch** row loses its
workspace and branch: fall back to `locus delegate run` for read-only dispatch, and say so.
Work the table routes to **native** is unaffected — that route does not depend on allele.

Never abandon routing and do the work inline because one mechanism is unavailable.
...
8. **Reflect in LEARN** — the four questions are answered in the visible response. Route anything that outlives the session to a store something reads: `{data}/projects/`, `DECISIONS.md`, or an issue. Do not file to `memory/learning/` (DEV-516).
